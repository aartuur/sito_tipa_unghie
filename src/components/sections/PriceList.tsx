      ...
      <AnimationWrapper>
        <div className="max-w-7xl mx-auto px-6">
          ...
          <AnimatePresence mode="wait"> {/* Use AnimatePresence for exit/enter animations */}
            {viewMode === 'table' ? (
              <motion.div
                key="table-view" // Key is essential for AnimatePresence
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -20 }}
                transition={{ duration: 0.3 }}
                className="bg-white/5 backdrop-blur-sm rounded-3xl p-6 md:p-10 border border-white/5 shadow-2xl min-h-[400px]"
              >
              ...
              <motion.div
                key="card-view" // Key is essential for AnimatePresence
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -20 }}
                transition={{ duration: 0.3 }}
              ...
              >
                {currentPricingData[activeTab]?.map((item, idx) => (
                  <AnimationWrapper key={idx} delay={idx * 0.1}>
                    <MUICard className="p-8 relative group overflow-hidden cursor-pointer flex flex-col h-full">
                      {/* Shine Effect */}
                      ...
                      <div className="relative z-10 flex flex-col h-full">
                        ...
                        <ul className="text-gray-400 text-sm space-y-2 mb-6 flex-grow">
                          {item.details?.map((detail, dIdx) => ( // Re-added optional chaining
